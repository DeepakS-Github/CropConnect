...
const extractVerificationTokenAndTypeFromLink = require('../helper/extractVerificationTokenAndTypeFromLink');
...
const runLoginTests = (type) => {
    ...
    type = type.toLowerCase();
    const loginDetails = type === 'seller' ? sellerLoginDetails : userLoginDetails;
    ...
    describe(`${capitalizeFirstLetter(type)} Login`, () => {
        ...
        if (type === 'seller' || type === 'user') {
            ...
            //         const res = await request(app).post(`/auth/${type}/login`).send(modifiedSignupDetails);
            //         expect(res.statusCode).toEqual(400);
            //         expect(res.body).toHaveProperty('message', `${capitalizeFirstLetter(type)} with this email already exists`);
            //     })
            // })
            ...
            //     describe('given the repeated seller brand name', () => {
            //         it('should return 409 and error message', async () => {
            //             const res = await request(app).post('/auth/seller/login').send({ ...sellerDetails, email: "nvccafpbxsnudwfaae@ytnhy.com", contact: 1234567899 });
            //             expect(res.statusCode).toEqual(409);
            //             expect(res.body).toHaveProperty('message', 'This brand name already exists');
            //         })
            //     })
